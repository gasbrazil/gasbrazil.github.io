...
.spark-gas-label { display: flex; align-items: center; justify-content: space-between; gap: 8px; }
...
function paintSparkUnit() {
  ...
  if (gasEl) gasEl.step = sparkUnit === "mmbtu" ? "0.01" : "0.001";
}

function setSparkUnit(unit) {
  if (!unit || unit === sparkUnit) return;
  const gasEl = document.getElementById("spark-gas");
  const cur = m3FromDisplay(gasEl.value);
  if (cur != null) sparkGasM3 = cur;
  sparkUnit = unit;
  gasEl.value = displayGasFromM3(sparkGasM3);
  paintSparkUnit();
  renderSpark();
}
...
function renderSpark() {
  ...
  const gas = m3FromDisplay(gasEl.value);
...
function initSparkForm() {
  ...
  const hrEl = document.getElementById("spark-hr");
  const preset = document.getElementById("spark-hr-preset");
  sparkGasM3 = (s.defaultGasPrice != null && !isNaN(s.defaultGasPrice)) ? s.defaultGasPrice : 1.2;
  sparkUnit = "mmbtu";
  gasEl.value = displayGasFromM3(sparkGasM3);
  hrEl.value = s.heatRateCcgt || 1800;
  preset.value = "ccgt";
  document.getElementById("spark-gas").addEventListener("input", () => {
    const cur = m3FromDisplay(document.getElementById("spark-gas").value);
    if (cur != null) sparkGasM3 = cur;
    renderSpark();
  });
  ["spark-hr", "spark-cvu"].forEach(id => {
