...
**Status:** Accepted (Tracks A–C scaffolded; Track B on Flows + ONS)  
**Date:** 2026-09-07  
...
The page shell stays on Pages. Large series are separate gzip artifacts the browser fetches (e.g. `flows/payload.json.gz`, `ons/payload.json.gz`). HTML keeps only chrome + hub teaser markers.
...
| B | First-load and git diffs shrink for Flows and ONS; pattern can extend to other heavy pages |
...
- Implementation: `shared/data_kit.py`, `shared/schemas/`, `lake/README.md`, `flows/payload.json.gz`, `ons/payload.json.gz`, `api/`
