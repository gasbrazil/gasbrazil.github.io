...
def _clip_volutil(v: float) -> float:
    """Usable volume is % of useful capacity — never show below 0 or above 100."""
...
def build_payload(df: pd.DataFrame, ent: pd.DataFrame) -> dict:
    ...
    for (s, sub, e), grp in df.groupby(["series", "subsystem", "entity"],
        ...
        for d, v in zip(grp["date"], grp["value"]):
            fv = float(v)
            if s == "res_volutil_pct":
                fv = _clip_volutil(fv)
            vals[idx[d]] = round(fv, dec)
        series[f"{s}|{sub}|{e}"] = vals

    if len(ent):
        ent = ent.copy()
        for col in ("kind", "subsystem", "entity"):
            if col in ent.columns:
                ent[col] = ent[col].astype(str).str.strip()
        ent = ent.drop_duplicates(subset=["kind", "subsystem", "entity"],
                                  keep="last")
...
/* KPI strip is always ONE row: grid-auto-flow:column lays every tile in its
   ...
   margin-bottom matches .card / sibling .kpi-row spacing so the strip does
   not sit flush against controls / Reservoirs summary cards below it. */
.tiles{display:grid;grid-auto-flow:column;grid-auto-columns:minmax(0,1fr);gap:8px}
#kpiTiles{margin-bottom:var(--gap)}
...
table.data tbody tr.row-link:hover{background:var(--accent-soft)}
table.data tbody tr.is-filtered{background:var(--accent-soft)}
.entlist tr.empty-res{opacity:.65}
.metric-cell{white-space:nowrap}
.metric-cell .band-label{margin-left:6px}
...
<div id="mixStrip" style="margin:0 0 var(--gap)"></div>
<div id="resSummary"></div>

<div class="card" id="controlsCard">
  <div class="controls">
    <div class="ctl"><label data-ons-i18n="dateRange">Date range</label><div class="row" id="presets"></div></div>
    <div class="ctl"><label data-ons-i18n="from">From</label><input type="date" id="from"></div>
    <div class="ctl"><label data-ons-i18n="to">To</label><input type="date" id="to"></div>
    <div class="ctl"><label data-ons-i18n="smoothing">Smoothing</label><div class="row" id="smooth"></div></div>
    <div class="ctl" id="subsCtl"><label data-ons-i18n="subsystems">Subsystems</label><div class="row" id="subs"></div></div>
    <div class="ctl"><label data-ons-i18n="view">View</label>
      <div class="row"><button id="tableBtn" aria-pressed="false" data-ons-i18n="table">Table</button></div>
    </div>
  </div>
</div>
...
/* ---------- views ---------------------------------------------------------- */
const VIEWS = [
  {id:"subsystems", labelKey:"viewSubsystems", kind:""},
  {id:"plants",     labelKey:"viewPlants", kind:"plant"},
  {id:"reservoirs", labelKey:"viewReservoirs", kind:"reservoir"},
  ...
  {id:"tables",     labelKey:"viewTables", kind:null},
];
...
const state = {
  ...
  showSubunits: false,
  // Off by default: hide reservoirs whose latest usable volume is 0% after
  // clipping ONS negatives / overflow to [0, 100]. Toggled via "Show empty"
  // on the Reservoirs picker — same pattern as showSubunits.
  showEmptyReservoirs: false,
...
const ONS_I18N = {
  en: {
    ...
    viewPlants: "Thermal plants",
    viewReservoirs: "Reservoirs",
    viewTables: "Data Tables",
    aboutView: "About this view",
    ...
    smooth7: "7d avg",
    smooth30: "30d avg",
    tipSubsystems: "National and per-subsystem balance: load, generation by source, hydrology and CMO, on a daily grid. Toggle SIN/SE/S/NE/N to fan any picked series across subsystems. Net interchange is positive when a subsystem is a net exporter.",
    tipPlants: "One row per dispatched thermal plant, filterable by region and fuel. The five pinned Total rows are synthetic subsystem/national aggregates of the gas-fired fleet, not plants, and are excluded from fleet-wide KPI sums. A combined-cycle block appears both as its individual dispatch phases and as one combined row — don't sum the two together. Installed capacity, utilization and estimated gas consumption are derived, not published by ONS: see Methodology in the footer for the heat-rate assumption behind the gas figures.",
    tipReservoirs: "Region EAR chart and region / REE / basin rollups above; individual reservoirs below. Usable volume is % of each reservoir's useful capacity (0–100), not a raw water volume. Empty or invalid readings (ONS sometimes publishes negatives) are omitted from averages and the picker. REEs with zero reported capacity (e.g. Itaipu) are omitted from the REE table. EAR is stored energy rather than raw volume, which is what makes cascades comparable. Reservoirs down usually means more thermal (and gas) later."
  },
  pt: {
    viewSubsystems: "Subsistemas",
    viewPlants: "Usinas térmicas",
    viewReservoirs: "Reservatórios",
    viewTables: "Tabelas",
    aboutView: "Sobre esta visão",
    dateRange: "Período",
    from: "De",
    to: "Até",
    ...
    smooth7: "Média 7d",
    smooth30: "Média 30d",
    tipSubsystems: "Balanço nacional e por subsistema: carga, geração por fonte, hidrologia e CMO, em grade diária. Alternar SIN/SE/S/NE/N espalha qualquer série escolhida pelos subsistemas. Intercâmbio líquido positivo = subsistema exportador líquido.",
    tipPlants: "Uma linha por usina térmica despachada, filtrável por região e combustível. As cinco linhas Total fixas são agregados sintéticos da frota a gás — não são usinas — e ficam de fora das somas de frota. Um bloco ciclo combinado aparece como fases e como linha combinada: não some as duas. Capacidade, utilização e consumo estimado de gás são derivados, não publicados pela ONS: ver Metodologia no rodapé.",
    tipReservoirs: "Gráfico EAR por região e totais por região / REE / bacia acima; reservatórios individuais abaixo. Volume útil é % da capacidade útil de cada reservatório (0–100), não um volume bruto. Leituras vazias ou inválidas (a ONS às vezes publica negativos) ficam de fora das médias e da lista. REEs com capacidade zero reportada (ex.: Itaipu) ficam de fora da tabela de REE. EAR é energia armazenada, não volume bruto — por isso as cascadas são comparáveis. Reservatórios baixos costumam antecipar mais térmica (e gás)."
  }
};
function ot(key) {
  const pack = ONS_I18N[currentLang()] || ONS_I18N.en;
  return pack[key] || ONS_I18N.en[key] || key;
}
function applyOnsI18n() {
  document.querySelectorAll("[data-ons-i18n]").forEach(el => {
    const key = el.getAttribute("data-ons-i18n");
    if (key) el.textContent = ot(key);
  });
}
...
function metricLabel(m){
  ...
  if(!meta) return m;
  const u=meta.unit, lab=meta.label;
  if(!u) return lab;
  if(lab.includes("("+u+")")) return lab;
  if(u==="%" && lab.includes("%")) return lab;
  return lab+" ("+u+")";
}
function uniqueByEntity(list){
  const seen=new Set(), out=[];
  (list||[]).forEach(e=>{
    const id=(e.subsystem||"")+"|"+(e.entity||"");
    if(seen.has(id)) return;
    seen.add(id); out.push(e);
  });
  return out;
}
function sanitizeUsableVolume(v){
  // ONS publishes usable volume as % of useful capacity. Negatives (below
  // min operating level, or a unit glitch) and readings well over 100 are
  // not a useful figure — floor at 0, cap at 100.
  if(v==null || !isFinite(v)) return null;
  if(v<0) return 0;
  if(v>100) return 100;
  return v;
}
function latestUsableVolume(e){
  const arr=DATA.series[skey("res_volutil_pct",e.subsystem,e.entity)]||[];
  let v=null;
  for(let k=arr.length-1;k>=0;k--) if(arr[k]!=null){ v=arr[k]; break; }
  return sanitizeUsableVolume(v);
}
function reservoirHasStorage(e){
  const v=latestUsableVolume(e);
  return v!=null && v>0;
}
...
function fullSeries(key){
  ...
  if(isVirtualTotal(s,e)){
    ...
    if(m==="res_volutil_pct") out=out.map(sanitizeUsableVolume);
  }
...
function buildTabs(){
  ...
  VIEWS.forEach(v=>{
    const b=el("button",null,ot(v.labelKey));
...
function buildSmooth(){
  ...
  [[ot("smoothDaily"),1],[ot("smooth7"),7],[ot("smooth30"),30]].forEach(([nm,w])=>{
    const b=el("button",null,nm);
...
function entityRows(){
  ...
  const all=uniqueByEntity(DATA.entities.filter(e=>e.kind===kind));
  ...
  const rest = (kind==="plant" ? all.filter(e=>!e.isTotal) : all)
    ...
    .filter(e=> kind!=="reservoir" || state.showEmptyReservoirs || reservoirHasStorage(e))
    ...
    .sort((a,b)=>{
      ...
      if(kind==="reservoir"){
        const va=latestUsableVolume(a), vb=latestUsableVolume(b);
        if(va==null && vb==null) return a.entity.localeCompare(b.entity);
        if(va==null) return 1;
        if(vb==null) return -1;
        if(va!==vb) return va-vb;   // lowest usable volume first
      }
...
function buildEntityPicker(card){
  ...
  if(kind==="reservoir"){
    ...
    title.appendChild(document.createTextNode("Individual reservoirs"));
    title.appendChild(infodot(
      "One row per reservoir ONS tracks. Usable volume is the percentage of that reservoir's own useful capacity (0% = empty, 100% = full), not a volume in hm³ or a share of the national total. Readings ONS publishes below 0 or above 100 are clipped. Reservoirs at 0% are hidden unless you turn on Show empty. Select rows to chart them.",
      "About individual reservoirs"));
    card.appendChild(title);
    card.appendChild(el("p","panel-note",
      "Latest usable volume (% of useful capacity) · lowest first · click a basin in the table above to scope this list."));
  }
  ...
  metrics.forEach(m=>{
    const b=el("button",null,metricLabel(m));
  ...
  if(kind==="reservoir"){
    const emptyBtn=el("button",null,
      state.showEmptyReservoirs?"Hide empty":"Show empty");
    emptyBtn.setAttribute("aria-pressed", String(state.showEmptyReservoirs));
    emptyBtn.title="Reservoirs at 0% usable volume (including ONS readings that were negative) are hidden by default so they don't pull basin averages down or masquerade as the lowest reservoir.";
    emptyBtn.onclick=()=>{
      state.showEmptyReservoirs=!state.showEmptyReservoirs;
      buildPickCard(); render();
    };
    selRow.appendChild(emptyBtn);
  }
...
function renderEntityList(){
  ...
  mset.forEach(m=>{
    ...
    inner.appendChild(el("span","th-label",metricLabel(m)));
  ...
  rows.forEach(e=>{
    ...
    mset.forEach(m=>{
      ...
      td.textContent=fmtNum(last,decOf(k));
      if(kind==="reservoir" && m==="res_volutil_pct" && last!=null){
        const band=el("span","band-label",bandLabel(last));
        band.style.color=bandColor(last);
        td.appendChild(document.createTextNode(" "));
        td.appendChild(band);
      }
      tr.appendChild(td);
    });
    if(kind==="reservoir" && !reservoirHasStorage(e)) tr.classList.add("empty-res");
...
function drawPanel(unit,title,keys,W,opts){
  opts=opts||{};
  const colorFn=opts.colorFn||(k=>colorOf(k));
  const labelFn=opts.labelFn||(k=>labelOf(k));
  const dates=windowDates();
  const cols=keys.map(k=>({key:k,vals:seriesValues(k),color:colorFn(k)}))
                 .filter(c=>c.vals.some(v=>v!=null));
  if(!cols.length) return null;

  const card=el("div","card");
  const decs = ["%","R$/MWh","m"].includes(unit) ? 1 : 0;
  card.innerHTML='<p class="panel-title">'+title+
    (opts.info? '<button class="infodot" type="button" aria-label="About this chart" '+
      'data-info="'+escapeHtml(opts.info)+'">i</button>' : '')+
    '</p><p class="panel-note">'+
    (state.smooth>1? state.smooth+"-day moving average" : "Daily values")+
    ' · '+dates[0]+' to '+dates[dates.length-1]+
    (opts.note? ' · '+opts.note : '')+'</p>';

  const H=330, ML=64, MT=12, MB=30;
  const direct = cols.length<=(opts.directMax||4), MR = direct?200:16;
  ...
  if(direct){
    ...
    marks.forEach(m=>{
      ...
      g.textContent=shorten(labelFn(m.c.key),28); svg.appendChild(g);
    });
  }
  ...
  function showTooltip(i,focusCols,clientX,clientY){
    ...
    focusCols.forEach(c=>{
      ...
      rows+='<tr><td><span class="sw" style="display:inline-block;background:'+
        c.color+';border-color:'+c.color+'"></span> '+escapeHtml(labelFn(c.key))+
  ...
  if(cols.length>=2){
    ...
    cols.forEach(c=>{
      ...
      s.innerHTML='<span class="sw" style="background:'+c.color+';border-color:'+
        c.color+'"></span>'+escapeHtml(labelFn(c.key));
      lg.appendChild(s);
    });
    card.appendChild(lg);
  }
...
function renderCharts(){
  ...
  DATA.unitPanels.forEach(p=>{
    ...
    if(state.view==="reservoirs"){
      if(p.unit==="%") panelTitle="Usable volume (% of useful capacity)";
      else if(p.unit==="m") panelTitle="Upstream level (metres)";
    }
    const c=drawPanel(p.unit,panelTitle,keys,W);
...
function buildAllDataSheets(){
  ...
  const resRows=[["Date","Reservoir","Subsystem","Basin","Usable volume (%)",
    "Upstream level (m)"]];
  uniqueByEntity((DATA.entities||[]).filter(e=>e.kind==="reservoir")).forEach(ent=>{
    const sub=ent.subsystem, name=ent.entity;
    ...
    dates.forEach((d,i)=>{
      const vol=volArr?sanitizeUsableVolume(volArr[i]):null, lvl=lvlArr?lvlArr[i]:null;
...
// shows and how to read its headline numbers. Copy lives in ONS_I18N so EN/PT
// stay in sync with the rest of the chrome.
const VIEW_INFO_KEYS={
  subsystems:"tipSubsystems",
  plants:"tipPlants",
  reservoirs:"tipReservoirs"
};
function renderViewInfo(){
  const hostv=document.getElementById("viewInfo");
  if(!hostv) return;
  hostv.innerHTML="";
  const key=VIEW_INFO_KEYS[state.view];
  if(!key){ hostv.hidden=true; return; }
  const text=ot(key);
  hostv.hidden=false;
  const wrap=el("div");
...
function reservoirExtremes(){
  const ents=uniqueByEntity((DATA.entities||[]).filter(e=>e.kind==="reservoir"));
  let lowest=null, below=0, total=0;
  ents.forEach(e=>{
    const v=latestUsableVolume(e);
    if(v==null || v<=0) return;          // empty / invalid: omit
    total++;
    if(v<20) below++;
...
function renderReservoirKpis(host, extra){
  ...
  if(ext.lowest){
    host.appendChild(kpiTile("Lowest individual reservoir", fmtNum(ext.lowest.v,1), "% of capacity",
  ...
  if(ext.total){
    host.appendChild(kpiTile("Reservoirs below 20%", ext.below+" of "+ext.total, "",
      "usable volume critically low · empty reservoirs omitted"));
  }
  if(extra){
    renderReservoirRegionTable(extra);
    renderReservoirRegionChart(extra);
    renderReeTable(extra);
    renderBasinSummary(extra);
  }
}
function subColor(sub){
  // Same fixed SIN/SE/S/NE/N hues as the Thermal Plants Total rows, so a
  // region's color means the same thing across tabs.
  const pal=isDark()?TOTAL_COLOR_DARK:TOTAL_COLOR_LIGHT;
  return pal[sub]||mixColor("hydro");
}
function renderReservoirRegionChart(host){
  // Always-on EAR% time series for the four published regions + SIN — the
  // market-level hydro picture the per-reservoir chart below cannot give.
  const order=["SIN","SE","S","NE","N"];
  const keys=order.map(s=>skey("ear_pct",s)).filter(exists);
  if(keys.length<2) return;
  let box=document.getElementById("resRegionChart");
  if(!box){
    box=el("div"); box.id="resRegionChart"; box.style.marginBottom="14px";
    host.appendChild(box);
  }else{
    box.innerHTML="";
    if(!box.parentNode) host.appendChild(box);
  }
  const W=Math.max(680,(host.clientWidth||box.clientWidth||900)-8);
  const card=drawPanel("%","Hydro reservoirs by region",keys,W,{
    colorFn:k=>subColor(k.split("|")[1]),
    labelFn:k=>{
      const s=k.split("|")[1];
      return DATA.subsystemLabels[s]||subShort(s);
    },
    directMax:5,
    note:"stored energy (EAR) as % of regional capacity",
    info:"Same EAR figures as the region table above, plotted over your date "+
      "range. This is the cascade-comparable stored-energy view of hydro — "+
      "not the per-reservoir usable-volume chart further down the page."
  });
...
function reeHasCapacity(r){
  // ONS sometimes lists an REE with 0 MWmês capacity (Itaipu today) — that
  // reads as "0.0% · 0 / 0" and is not a real storage figure to rank or chart.
  return r && r.pct!=null && r.cap!=null && r.cap>0;
}
function renderReeTable(host){
  const ents=uniqueByEntity((DATA.entities||[]).filter(e=>e.kind==="ree"));
  if(!ents.length) return;
  const all=ents.map(reeRow).filter(r=>r && r.pct!=null);
  const omitted=all.filter(r=>!reeHasCapacity(r)).length;
  const rows=all.filter(reeHasCapacity).sort((a,b)=>a.pct-b.pct);
  ...
  let h='<p class="panel-title">EAR by Reservoir Equivalent (REE)'+
    ...
    'opposite directions underneath one steady regional number. REEs with zero '+
    'reported capacity are omitted.">i</button></p>'+
    '<p class="panel-note">Lowest stored-energy % first. Finer than the region table, coarser than a single reservoir.'+
    (omitted? ' · '+omitted+' zero-capacity REE'+(omitted===1?'':'s')+' omitted.':'')+
    '</p>';
...
function renderReservoirRegionTable(host){
  ...
  if(!rows.length) return;
  const card=el("div","card"); card.style.marginBottom="14px";
  const f=state.filter.reservoirs||{};
  let h='<p class="panel-title">Hydro reservoirs by region'+
    ...
    'potential per unit of water. Click a region (not Total) to list its reservoirs below.">i</button></p>'+
    '<p class="panel-note">Stored energy (EAR), % of regional capacity · click a region to filter the list below.</p>';
  h+='<div class="scroll"><table class="data"><thead><tr>'+
  ...
  rows.forEach(r=>{
    ...
    const w=Math.max(0,Math.min(100,r.pct));
    const clickable=r.sub!=="SIN";
    const active=clickable && f.region===r.sub && !f.group;
    h+='<tr'+(clickable?' class="row-link'+(active?' is-filtered':'')+'" data-sub="'+
      escapeHtml(r.sub)+'" title="List reservoirs in this region"':'')+'>'+
  ...
  card.querySelectorAll("tr.row-link").forEach(tr=>{
    tr.addEventListener("click", ()=>{
      const filt=state.filter.reservoirs;
      filt.region=tr.dataset.sub;
      filt.group="";
      onFilterChange();
      const pick=document.getElementById("pickCard");
...
function renderBasinSummary(host){
  ...
  const f=state.filter.reservoirs||{};
  const card=el("div","card");
  let h='<p class="panel-title">Usable volume by basin'+
    '<button class="infodot" type="button" aria-label="About usable volume" '+
    'data-info="Usable volume is the percentage of each reservoir’s own useful '+
    'capacity (0% empty – 100% full), then averaged across the reservoirs in that '+
    'basin. It is not stored energy (EAR) and not a volume in hm³. Empty or invalid '+
    'ONS readings are omitted so a glitch like −1,900% cannot pull a basin average '+
    'negative. Click a row to list that basin’s reservoirs below.">i</button></p>'+
    '<p class="panel-note">Latest usable volume (% of useful capacity) per reservoir, '+
    'averaged by basin · lowest first · click a row to list those reservoirs.'+
    (omitted? ' · '+omitted+' empty reservoir'+(omitted===1?'':'s')+' omitted.':'')+
    '</p>';
  h+='<div class="scroll"><table class="data"><thead><tr>'+
    '<th class="l">Basin</th><th class="l">Region</th><th>Reservoirs</th>'+
    '<th class="l">Capacity filled</th>'+
    '<th>Avg (%)</th><th>Min (%)</th><th>Max (%)</th></tr></thead><tbody>';
  rows.forEach(r=>{
    const w=Math.max(0,Math.min(100,r.avg));
    const active=f.region===r.sub && f.group===r.basin;
    h+='<tr class="row-link'+(active?' is-filtered':'')+'" data-sub="'+
      escapeHtml(r.sub)+'" data-basin="'+escapeHtml(r.basin)+
      '" title="List reservoirs in this basin">'+
      '<td class="l">'+escapeHtml(r.basin)+'</td>'+
      '<td class="l">'+escapeHtml(DATA.subsystemLabels[r.sub]||r.sub)+'</td>'+
      '<td>'+r.n+'</td>'+
      '<td class="l"><div class="cap-bar" title="'+fmtNum(r.avg,1)+'% full">'+
        '<div style="width:'+w+'%;background:'+bandColor(r.avg)+'"></div></div> '+
        '<span class="band-label" style="color:'+bandColor(r.avg)+'">'+bandLabel(r.avg)+
        '</span></td>'+
  ...
  card.querySelectorAll("tr.row-link").forEach(tr=>{
    tr.addEventListener("click", ()=>{
      const filt=state.filter.reservoirs;
      filt.region=tr.dataset.sub;
      filt.group=tr.dataset.basin==="—" ? "" : tr.dataset.basin;
      onFilterChange();
      const pick=document.getElementById("pickCard");
      if(pick) pick.scrollIntoView({behavior:"smooth", block:"start"});
    });
...
async function boot(){
  ...
  document.getElementById("foot").innerHTML =
    ...
    '<b>Reservoir usable volume.</b> The hydraulic file\u2019s '+
    '<code>val_volumeutilcon</code> is a percentage of each reservoir\u2019s useful '+
    'capacity, not a volume in hm\u00b3. Readings below 0 (below the minimum operating '+
    'level, or a data glitch) and above 100 are clipped to that range. Reservoirs '+
    'whose latest reading is 0% are omitted from basin averages, the lowest-reservoir '+
    'KPI, and the picker (toggle Show empty to include them).<br><br>'+
  ...
  let rz; window.addEventListener("resize",()=>{
    clearTimeout(rz); rz=setTimeout(()=>{
      renderCharts();
      if(state.view==="reservoirs"){
        const host=document.getElementById("resSummary");
        if(host) renderReservoirRegionChart(host);
      }
    },140);
  });
  ...
  // returns its rows selected-first then (plants) alphabetically by name
  // or (reservoirs) lowest usable volume first.
