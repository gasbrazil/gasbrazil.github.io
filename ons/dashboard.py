...
const ONS_I18N = {
  en: {
    ...
    tipReservoirs: "Individual reservoirs below, with region / REE / basin rollups above. Usable volume is % of each reservoir's useful capacity (0–100), not a raw water volume. Empty or invalid readings (ONS sometimes publishes negatives) are omitted from averages and the picker. EAR is stored energy rather than raw volume, which is what makes cascades comparable. Reservoirs down usually means more thermal (and gas) later."
  },
  pt: {
    ...
    tipReservoirs: "Reservatórios individuais abaixo, com totais por região / REE / bacia acima. Volume útil é % da capacidade útil de cada reservatório (0–100), não um volume bruto. Leituras vazias ou inválidas (a ONS às vezes publica negativos) ficam de fora das médias e da lista. EAR é energia armazenada, não volume bruto — por isso as cascadas são comparáveis. Reservatórios baixos costumam antecipar mais térmica (e gás)."
  }
};
function ot(key) {
...
function renderReeTable(host){
  ...
  let h='<p class="panel-title">EAR by Reservoir Equivalent (REE)'+
    ...
    '<p class="panel-note">Lowest stored-energy % first. Finer than the region table, coarser than a single reservoir.</p>';
